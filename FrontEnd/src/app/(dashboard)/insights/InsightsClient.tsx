...
interface InsightsClientProps {
  ...
  monthlyTrendData: { month: string; emissions: number }[]
...
export default function InsightsClient({
  ...
  const monthlyGoal = 100 // kg CO2 target
  const currentMonthEmissions = monthlyTrendData[monthlyTrendData.length - 1]?.emissions || 0
  const goalProgress = monthlyGoal > 0 ? Math.min(100, Math.round((1 - currentMonthEmissions / monthlyGoal) * 100)) : 0
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-emerald-600">CO₂ this week</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{Math.max(0, goalProgress)}%</div>
            <p className="text-xs text-slate-500">toward monthly target</p>
          </CardContent>
        </Card>
      </div>
